...
export function useAnalysisDraft({
  ...
  const flushLatestDraft = useCallback(() => {
    ...
    if (!hasMeaningfulAnalysisDraftData(payload.form, payload.selectedCustomerId, payload.draftId)) {
  ...
  }, [form, selectedCustomerId, step, draftId, state.status, scope, flushLatestDraft]);
