...
import { resolveHouseholdSize } from "@/services/debtRelief";
...
export function useDiagnosisForm() {
  ...
  const derived: DiagnosisDerivedValues = useMemo(() => {
    ...
    const totalDebtWon = form.debts.reduce(
      (sum, debt) => sum + (debt.currentBalanceWon || 0),
      0
    );

    // 법원 인정 기준 월 가용소득 = 월소득 − 가구원수별 법원 인정 최저생계비 − 추가 인정 고정지출
    // (서버 disposableIncome과 동일 공식 — 2026-08-07 스펙). 모든 값은 원 단위다.
    ...
    const monthlyAvailableIncomeWon =
      ...
      form.additionalFixedExpense;

    return {
      totalDebtWon,
      monthlyIncomeWon: form.monthlyIncome ?? 0,
      minimumLivingCostWon,
      householdSize,
      monthlyAvailableIncomeWon,
    };
  }, [form]);
