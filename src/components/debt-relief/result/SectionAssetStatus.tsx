...
function buildAssetComposition(detail: DiagnosisDetail): AssetCompositionItem[] {
  ...
  return ASSET_CATEGORY_OPTIONS.flatMap((option) => {
    const amountManwon = amounts.get(option.value) ?? 0;
    if (amountManwon <= 0) return [];
    return [{
      category: option.value,
      label: option.label,
      amountManwon,
      percent: Math.round((amountManwon / totalAmount) * 100),
    }];
  });
}
      ...
      {composition.length === 0 ? (
        ...
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-2 lg:gap-10">
          <div className="grid min-w-0 grid-cols-2 gap-5 md:pl-3 md:gap-x-12">
            <AssetMetric
              label="총 자산"
              value={detail.debtStatus.totalAssetManwon.toLocaleString("ko-KR")}
            />
            <AssetMetric
              label="월 가용 소득"
              value={`${detail.debtStatus.monthlyAvailableIncomeManwon >= 0 ? "+" : ""}${detail.debtStatus.monthlyAvailableIncomeManwon.toLocaleString("ko-KR")}`}
            />
          </div>
