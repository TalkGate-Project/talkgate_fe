...
export default function ResultDetailContent({ diagnosisId }: { diagnosisId: string }) {
  ...
  // 스크롤 스파이: 화면 상단에 걸린 섹션을 활성 처리 (고정 헤더 54px 고려)
  useEffect(() => {
    if (!detail) return;
    const observer = new IntersectionObserver(
      ...
      // 전역 헤더(54px) + 서브헤더(48px) 아래로 진입한 섹션을 활성 처리
  ...
  return (
    <>
      {/* 전역 헤더 아래 고정되는 전체 폭 서브헤더 (콘텐츠 컨테이너 밖) */}
      <ResultAnchorNav sections={sections} activeId={activeId} onNavigate={scrollTo} />

      {/* fixed 서브헤더(48px) + 허브와 비슷한 상단 여백(36px) → 84px */}
      <div className="mx-auto max-w-[1324px] w-full px-0 md:px-6 lg:px-0 pt-[84px] pb-[calc(3rem+env(safe-area-inset-bottom))] flex flex-col gap-3 md:gap-9">
      {/* 헤더 + AI 분석 추천 (같은 카드) */}
