...
function InvitedMemberRow({
  ...
  const invitationDate = new Date(member.createdAt)
  ...
  return (
    <>
      {/* Desktop View */}
      <div className="hidden md:flex items-center px-10 py-4 w-full">
        {/* Email & Role - 가장 넓게 */}
          ...
          <div
            className={`text-[14px] ${
              isExpired
                ...
                : "text-foreground dark:text-neutral-80"
            }`}
          >
            {expirationDate}
          </div>
          {isExpired && (
            <div className="text-[12px] text-red-500 dark:text-red-500 mt-0.5">
              만료됨
            </div>
          )}
        </div>
      ...
      <div className="md:hidden flex items-center py-3 pl-0.5 pr-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="text-[14px] text-foreground dark:text-neutral-80 font-semibold truncate">
            {member.email}
          </div>
        </div>

        {/* 만료일시 열 */}
        <div className="w-[100px] flex-none text-left">
          <div
          ...
          {isExpired && (
            <div className="text-[11px] text-red-500 dark:text-red-500 mt-0.5">
              만료됨
            </div>
          )}
        </div>
...
export default function InvitedMemberSettings() {
  ...
  const { data: invitationsData, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const response = await MembersService.listInvitations({
        page: currentPage,
        limit: 10,
      });
  ...
  const resendMutation = useMutation({
    mutationFn: (invitationId: number) =>
      MembersService.resendInvitation(invitationId),
    onSuccess: () => {
  ...
  const cancelMutation = useMutation({
    mutationFn: (invitationId: number) =>
      MembersService.cancelInvitation(invitationId),
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: ["invitations", "list", projectId],
      });
  ...
  if (!projectId) {
    return (
      <div className="bg-card rounded-[14px] p-6">
        <p className="text-neutral-60 dark:text-neutral-60">
          프로젝트를 선택해주세요.
        </p>
      </div>
    );
  ...
  return (
    <div className="bg-card rounded-[14px] lg:rounded-[14px] rounded-t-none lg:rounded-t-[14px] pb-4 md:pb-7 min-h-[70dvh]">
      {/* Header */}
      ...
      {/* Divider */}
      <div className="w-full h-[1px] bg-border dark:bg-neutral-30 opacity-70 mb-4 md:mb-6 px-4 md:px-7"></div>

      {/* content */}
      <div className="px-4 md:px-7">
        {/* Table Header - 모바일/데스크탑 분리 */}
        {/* Mobile Header */}
        <div className="md:hidden bg-neutral-20 dark:bg-neutral-20 rounded-[8px] px-4 h-[40px] flex items-center">
          <div className="flex items-center w-full">
            ...
            <div className="w-[100px] flex-none text-[14px] font-medium text-neutral-60 dark:text-neutral-60 text-left">
              만료일시
            </div>
