...
export default function AssignMemberTable() {
  const baseRows = [
    { name: '김영업', team: 'A팀', color: '#ADF6D2', count: 25 },
    { name: '이마케팅', team: 'B팀', color: '#FFDE81', count: 35 },
    { name: '박세일즈', team: 'A팀', color: '#ADF6D2', count: 31 },
    { name: '최고객', team: 'C팀', color: '#FC9595', count: 20 },
    { name: '오과장', team: 'B팀', color: '#FFDE81', count: 3 },
    { name: '박사원', team: 'A팀', color: '#ADF6D2', count: 12 },
    { name: '배정되지않음', team: '배정되지않음', color: '#7EA5F8', count: 15 },
  ];
  const router = useRouter();
  const searchParams = useSearchParams();
  const [open, setOpen] = useState(false);
  const [teamFilter, setTeamFilter] = useState<MemberFilterState['team']>((searchParams.get('assignTeam') as any) || 'all');
  const [sortOrder, setSortOrder] = useState<MemberFilterState['sort']>((searchParams.get('assignSort') as any) || 'desc');

  useEffect(() => {
    const params = new URLSearchParams(searchParams.toString());
    if (teamFilter && teamFilter !== 'all') params.set('assignTeam', teamFilter); else params.delete('assignTeam');
    if (sortOrder && sortOrder !== 'desc') params.set('assignSort', sortOrder); else params.delete('assignSort');
  ...
  const rows = useMemo(() => {
    ...
    r.sort((a,b)=> sortOrder==='desc' ? b.count - a.count : a.count - b.count);
    return r;
  }, [teamFilter, sortOrder]);
  ...
  return (
    <div className="mt-5">
      ...
      <div className="divide-y divide-[#E2E2E2]">
        {rows.map((r) => (
          <div key={`${r.name}-${r.team}`} className="h-[56px] grid grid-cols-3 items-center px-6">
