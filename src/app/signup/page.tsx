...
export default function SignupPage() {
  ...
  return (
    ...
    >
      ...
      >
        <div className="mx-auto flex flex-col items-center pt-[330px] w-[340px]" aria-label="signup-area">
          {/* 워드마크 로고 (로그인 페이지와 동일) */}
          ...
          {step === "account" && (
            ...
            >
              ...
              <label className={`block text-[12px] mb-1 ${invalid && !emailValid ? "text-[#FF5A5A]" : "text-[#CECECE]"}`}>이메일</label>
              <div className="flex gap-2">
                <input
                  ...
                  placeholder={invalid && !emailValid ? "이메일을 다시 입력하세요" : "이메일을 입력하세요"}
                  className={`flex-1 h-[40px] rounded-[5px] border bg-transparent px-3 text-white ${(invalid && !emailValid) ? "border-[#FF5A5A] placeholder-[#FF5A5A]" : "border-[#555555]"}`}
                  autoComplete="email"
                />
                <button
                  type="button"
                  className={`px-3 rounded-[5px] ${emailChecked ? "bg-[#1B5E20] text-white" : "bg-[#2F2F2F] text-[#D0D0D0]"} text-[13px]`}
                  onClick={() => {
                    if (!emailValid) {
                      setInvalid(true);
                      return;
                    }
                    SignupService.checkEmailAvailable({ email }).then(() => setEmailChecked(true));
                  }}
                >
                  {emailChecked ? "사용가능" : "중복확인"}
                </button>
              </div>
          ...
          {step === "profile" && (
            <form
              ...
              onSubmit={(e) => {
                e.preventDefault();
                setInvalid(false);
                if (!name || !phone || !code || !agreeTerms || !agreePrivacy) {
                  setInvalid(true);
                  return;
                }
            ...
            >
              ...
              <div className="mt-2">
                ...
                <div className="mt-2 pl-6 space-y-2">
                  <div className="flex items-center gap-2 text-[13px] text-[#BFBFBF]">
                    <Checkbox checked={agreeTerms} onChange={setAgreeTerms} ariaLabel="이용약관 동의" />
                    <span>이용약관에 동의합니다</span>
                  </div>
                  <div className="flex items-center gap-2 text-[13px] text-[#BFBFBF]">
                    <Checkbox checked={agreePrivacy} onChange={setAgreePrivacy} ariaLabel="개인정보 처리방침 동의" />
              ...
              <button
                type="submit"
                className="mt-2 w-full h-[40px] rounded-[5px] bg-[#252525] text-[#D0D0D0] text-[14px] font-semibold disabled:opacity-50"
                disabled={!name || !phone || !code || !agreeTerms || !agreePrivacy}
              >
                시작하기
              </button>
            </form>
